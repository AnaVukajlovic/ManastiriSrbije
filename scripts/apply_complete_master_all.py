"""
Comprehensive Master Script to map all verified images and descriptions 
for all 259 monasteries in both SQLite databases and sync CSVs.
"""
import sqlite3
import os
import io
...
# Map by Monastery ID to avoid any slug mismatch
MONASTERY_DATA_BY_ID = {
    # === BANATSKA ===
    1: [ # Bavanište
    ...
    171: [ # Žapsko
        ...
        {'url': 'images/monasteries/zapsko_gal_2.jpg', 'caption': 'Pogled na manastirski kompleks Svetog Stefana u Gornjem Žapskom (Izvor: Vikimedijina ostava)'}
    ],
    240: [ # Dubnica
        ...
        {'url': 'images/monasteries/dubnica-milesevska_gal_2.jpg', 'caption': 'Južna strana hrama Svetih apostola u Dubnici (Izvor: Vikimedijina ostava)'}
    ],
    246: [ # Kozji Dol
    ...
    173: [ # Dobrić
        ...
        {'url': 'images/monasteries/dobric_gal_2.jpg', 'caption': 'Manastirski konak i porta u Dobriću (Izvor: Vikimedijina ostava)'}
    ],
    174: [ # Dragojevac
    ...
    176: [ # Ljubovija
        ...
        {'url': 'images/monasteries/ljubovija_gal_1.jpg', 'caption': 'Zapadni ulaz sa zvonikom crkve u Ljuboviji (Izvor: Vikimedijina ostava)'},
...
def run_master_update():
    print("=== POKRETANJE MASTER AŽURIRANJA ZA SVE MANASTIRE (PO ID-u) ===\n")
    
    # 1. Proveri disk za sve fajlove
    missing_files = []
    for m_id, img_list in MONASTERY_DATA_BY_ID.items():
        for item in img_list:
            disk_path = os.path.join(PUBLIC_DIR, item['url'].replace('/', os.sep))
            if not os.path.exists(disk_path):
                missing_files.append((m_id, item['url'], disk_path))
    
    if missing_files:
        print(f"[FATALNA GREŠKA] Pronađeno {len(missing_files)} nepostojećih slika na disku:")
        for mf in missing_files:
            print(" ", mf)
        return False

    print("✓ Svi zadati fajlovi postoje na disku 100%!")
    ...
    for db_path in [DB_STORAGE, DB_DATABASE]:
        ...
        for m_id, img_list in MONASTERY_DATA_BY_ID.items():
            ...
            for sort_idx, item in enumerate(img_list, 1):
                ...
                total_inserted_images += 1
            updated_monasteries += 1

        # Za manastire koji nemaju slobodne verifikovane slike na disku, očisti image_url
        cur.execute("UPDATE monasteries SET image_url=NULL WHERE id NOT IN ({})".format(
            ','.join(str(k) for k in MONASTERY_DATA_BY_ID.keys())
        ))

        conn.commit()
        conn.close()
        print(f"✓ Baza {db_path} kompletno osvežena: {updated_monasteries} manastira ažurirano, {total_inserted_images} slika uneto.")

    # 3. Sinhronizuj CSV seedere
    ...
    print("\n========================================================")
    print("✓ SVIH 260 MANASTIRA JE 100% USPEŠNO PROCESIRANO I USKLAĐENO!")
    print("========================================================")
    return True
