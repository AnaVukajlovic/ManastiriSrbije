...
test_slugs = ['bogorodica-ljeviska', 'studenica', 'tumane', 'manasija', 'zica', 'visoki-decani', 'gracanica', 'pecka-patrijarsija', 'banjska', 'devic', 'beocin', 'tresije']
...
for slug in test_slugs:
    try:
        ...
        if m_list:
            ...
            print(f"  Total Photos in Gallery: {len(gallery_json)}")
            for i, item in enumerate(gallery_json[:4], 1):
                print(f"    [{i}] {item.get('caption')} -> {item.get('url')[:65]}...")
        else:
