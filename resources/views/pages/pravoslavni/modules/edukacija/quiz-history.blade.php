...
<section class="section quiz-history-page">
    <div class="container">
        <header class="quiz-header">
            ...
            <h1 class="quiz-title">Put kroz srpsku istoriju</h1>
            <p class="quiz-desc">Testiraj svoje poznavanje Nemanjića i srednjovekovnih manastira.</p>
        </header>
        ...
        <div class="quiz-result-banner">
            <div class="result-content">
                ...
                <button type="button" class="btn-restart" onclick="restartQuiz()">⟳ Generiši novi kviz</button>
            </div>
        </div>
        @endif
        ...
        <form method="POST" action="{{ route('edukacija.quiz-history.submit') }}">
            ...
            <div class="quiz-grid">
                @foreach($questions as $i => $q)
                    {{-- U slučaju da je AI vratio niz objekata, koristimo ?. operator za sigurnost --}}
...
async function askAi(btn, text) {
    ...
    box.innerHTML = "<em>Savetnik analizira istorijske arhive...</em>";
...
async function restartQuiz() {
    ...
    await fetch('{{ route('edukacija.ai.regenerate') }}', { 
        ...
        body: JSON.stringify({ type: 'history' }) // <--- OVO JE KLJUČNO
    });
    window.location.href = "{{ route('edukacija.quiz-history') }}";
}
</script>
@endsection